...
const getStatistics = expressAsyncHandler(async (req, res) => {
  try {
    ...
    const studentGirls =
      studentGenderCounts.find((g) => g._id === "F")?.count || 0;
    const studentBoys =
      studentGenderCounts.find((g) => g._id === "M")?.count || 0;
    const studentTotal = studentGirls + studentBoys;
    const studentGirlsPercentage = (
      (studentGirls / studentTotal) *
      100
    ).toFixed(2);
    const studentBoysPercentage = ((studentBoys / studentTotal) * 100).toFixed(
      2
    );

    const teacherGirls =
      teacherGenderCounts.find((g) => g._id === "F")?.count || 0;
    const teacherBoys =
      teacherGenderCounts.find((g) => g._id === "M")?.count || 0;
    const teacherTotal = teacherGirls + teacherBoys;
    const teacherGirlsPercentage = (
      (teacherGirls / teacherTotal) *
      100
    ).toFixed(2);
    const teacherBoysPercentage = ((teacherBoys / teacherTotal) * 100).toFixed(
      2
    );
    ...
    for (const grade of grades) {
      ...
      for (const student of students) {
        ...
        const validScores = scores.filter(
          (score) => score.subjectScoreId !== null
        );
        ...
        for (const subjectId in scoresBySubject) {
          ...
          const totalExamGrade = subjectScores.reduce(
            (sum, score) => sum + score.examGrade,
            0
          );
      ...
      const passedPercentage = (
        (passedStudents / totalStudentsInGrade) *
        100
      ).toFixed(2);
      const failedPercentage = (
        (failedStudents / totalStudentsInGrade) *
        100
      ).toFixed(2);
  ...
  try {
    ...
    for (const grade of grades) {
      ...
      const presentPercentage = (
        (presentCount / totalStudentsInGrade) *
        100
      ).toFixed(2);
      const absentPercentage = (
        (absentCount / totalStudentsInGrade) *
        100
      ).toFixed(2);
