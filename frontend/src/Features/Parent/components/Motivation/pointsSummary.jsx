...
const PointsSummary = () => {
  ...
  // Initialize active student from local storage or props
  useEffect(() => {
    // Get student ID from your application context
    const storedStudent = JSON.parse(localStorage.getItem("currentStudent"));
    
    if (storedStudent) {
      dispatch(setActiveStudent(storedStudent));
    } else {
  ...
  // Fetch rewards when activeStudent is set
  useEffect(() => {
  ...
  return (
    <div className="mt-6 p-6" dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      <h1 className="relative cursor-text bg-gradient-to-r from-[#FD813D] via-[#CF72C0] to-[#BC6FFB] bg-clip-text py-2 font-poppins text-2xl font-bold text-transparent dark:from-[#CE4EA0] dark:via-[#BF4ACB] dark:to-[#AE45FB]">
        {t("points.title")}
        <span className={`absolute bottom-[-9px] h-1 w-32 rounded-t-full bg-gradient-to-r from-[#FD813D] via-[#CF72C0] to-[#BC6FFB] ${
