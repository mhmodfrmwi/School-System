...
import { updateParentProfile, fetchParentData } from '../components/ParentRedux/ParentEditProfileSlice';
...
const ParentEditProfile = () => {
    ...
    const {
        ...
        error,
        success
    } = useSelector((state) => state.parentProfile || {});
    ...
    useEffect(() => {
        if (parentData && parentData.data) {
            ...
            setProfile({
                ...
                gender: parent.gender === "M" ? "Male" : parent.gender === "F" ? "Female" : "Other",
                phone: parent.phone || "",
                email: parent.email || "",
                role: "Parent",
                currentPassword: "",
            ...
            if (parent.profileImage) {
                setProfileImage(parent.profileImage);
            } else {
    ...
    const handleSave = () => {
        ...
        if (isEditingPassword) {
            if (profile.newPassword !== profile.confirmPassword) {
                toast.error(t('editProfile.passwordMismatch'));
                return;
            }
        ...
        dispatch(updateParentProfile(formData)).then(() => {
            setSelectedImage(null);
        });
    };

    if (fetchStatus === 'loading') {
        return <div className="text-center py-10 dark:text-gray-300">{t('editProfile.loading')}</div>;
    }

    if (fetchStatus === 'failed') {
        return <div className="text-center py-10 text-red-500 dark:text-red-400">{fetchError || t('editProfile.loadFailed')}</div>;
    }
