...
function AddStudent() {
  ...
  const handleBulkUpload = async () => {
    ...
    if (failedStudents.length > 0) {
      toast.error(`Failed to add the following students: ${failedStudents.join(", ")}`);
    }
  };
  ...
  return (
    <div className="relative mx-auto my-10 w-[80%] font-poppins">
      <h1 className="pl-5 text-2xl font-semibold text-[#244856]">Add Student</h1>
      <div className="ml-3 mt-1 h-[4px] w-[160px] rounded-t-md bg-[#244856]"></div>
      <div className="rounded-3xl bg-[#F5F5F5] p-6 shadow-md">
        <form onSubmit={handleAddStudent} className="m-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="col-span-1 sm:col-span-2">
            <button type="submit" className="text-md mx-auto block rounded-md bg-[#117C90] px-6 py-2 font-medium text-white transition hover:bg-[#0f6b7c]">
              Add Student
            </button>
          </div>
        </form>
        ...
        <div className="mt-6">
          <h2 className="text-xl font-semibold text-gray-700">Upload Excel File</h2>
          <input type="file" accept=".xlsx, .xls" onChange={handleFileUpload} className="mt-2 block w-full" />
          <button
            onClick={handleBulkUpload}
            className="mt-4 text-md mx-auto block rounded-md bg-[#117C90] px-6 py-2 font-medium text-white transition hover:bg-[#0f6b7c]"
          >
            Upload Students
          </button>
        </div>
