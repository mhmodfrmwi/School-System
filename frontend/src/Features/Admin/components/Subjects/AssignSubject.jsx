...
function AssignSubject() {
  ...
  const handleSemesterChange = (e) => {
    const selectedSemester = semesters.find(semester => {
      return `${semester.semesterName}, ${semester.academicYear_id.startYear}-${semester.academicYear_id.endYear}` === e.target.value;
    });
    if (selectedSemester) {
      setSemesterName(selectedSemester.semesterName);
      setAcademicYear(`${selectedSemester.academicYear_id.startYear}-${selectedSemester.academicYear_id.endYear}`);
    }
  };
      ...
      <div className="bg-[#F5F5F5] shadow-md p-6 rounded-3xl">
        <form onSubmit={handleAssignSubject} className="grid grid-cols-1 sm:grid-cols-2 gap-4 m-6">
          ...
          <div className="mb-4 sm:col-span-2">
            <label className="block text-md font-poppins font-medium text-gray-700 mb-2">Select Subject</label>
            <select
              value={subjectName}
          ...
          {/* Grade Dropdown */}
          <div className="mb-4 sm:col-span-2">
            <label className="block text-md font-medium text-gray-700 mb-2">Select Grade</label>
            <select
              value={gradeName}
            ...
            >
              ...
              {grade.map((grade, index) => (
                <option key={index} value={grade.gradeName}>{grade.gradeName}</option>
              ))}
            </select>
          </div>
          ...
          {/* Semester Dropdown */}
          <div className="mb-4 sm:col-span-2">
            <label className="block text-md font-medium text-gray-700 mb-2">Select Semester</label>
            <select
            ...
            >
              ...
              {semesters.map((semester, index) => (
                <option key={index} value={`${semester.semesterName}, ${semester.academicYear_id.startYear}-${semester.academicYear_id.endYear}`}>
