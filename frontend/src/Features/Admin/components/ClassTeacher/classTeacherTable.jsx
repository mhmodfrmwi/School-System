...
const ClassTeacherTable = () => {
  ...
  const filteredClassTeachers = classTeachers.filter((classTeacher) => {
    ...
    if (filterOption) {
      if (filterOption === "class") {
        return classTeacher.className?.toLowerCase().includes(lowerSearchText);
      }
      if (filterOption === "subject") {
        return classTeacher.subjectName?.toLowerCase().includes(lowerSearchText);
      }
      if (filterOption === "teacher") {
        return classTeacher.teacherName?.toLowerCase().includes(lowerSearchText);
      }
    ...
    return (
      ...
      classTeacher.teacherName?.toLowerCase().includes(lowerSearchText) ||
      classTeacher.startYear
        ?.toString()
        .toLowerCase()
        .includes(lowerSearchText) ||
      classTeacher.endYear?.toString().toLowerCase().includes(lowerSearchText)
    );
  });
  ...
  const handleDelete = async (classTeacherId) => {
    const confirmDelete = window.confirm(t("teacherTable.deleteConfirmation1"));
    if (confirmDelete) {
      try {
  ...
  const getClassDetails = useMemo(() => {
    return (classTeacher) => {
      ...
      return {
        className: t("teacherdata.Classnotfound"),
        gradeName: t("teacherdata.NoGrade"),
      };
    };
  }, [t]);
  ...
  const handleExportCSV = () => {
    const dataToExport = filteredClassTeachers.map((classTeacher) => ({
      [t("tableHeaders.Class")]: `${getClassDetails(classTeacher).className} - ${
        getClassDetails(classTeacher).gradeName
      }`,
      [t("tableHeaders.subject")]:
        classTeacher.subjectName || t("teacherdata.NoSubject"),
  ...
  if (loading) {
    return <div className="h-full w-full">{t("loading")}</div>;
  }
              ...
              <tr>
                <th
                  className={`px-3 py-2 text-${
                    isRTL ? "right" : "left"
                  } font-poppins text-xs font-medium sm:text-sm md:text-base`}
                >
              ...
              {paginatedClassTeachers.length > 0 ? (
                paginatedClassTeachers.map((classTeacher, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">{classTeacher.subjectName}</td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2">
                      ...
                      >
                        <Link
                          to={`/admin/edit-class-teacher/${id}/${classTeacher.classTeacherId}`}
