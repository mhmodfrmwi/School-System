            ...
            <tr className="border-b border-gray-200">
              <th colSpan="10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-3">
                  <div>
                    <span className="text-gray-600 text-lg">{t('examResults.summary.totalMarks')}: </span>
                    <span className="text-lg font-semibold text-gray-800">{examResult.result.total_marks}</span>
                  </div>
                  <div>
                    <span className="text-gray-600 text-lg">{t('examResults.summary.percentage')} : </span>
                    <span className="text-lg font-semibold text-gray-800">{parseFloat(examResult.result.percentage).toFixed(2)}%</span>
                  </div>
                  <div>
                    <span className="text-gray-600 text-lg">{t('examResults.summary.status')} : </span>
                    <span
          ...
          <tbody className="bg-[#F9F9F9]">
            <div className="w-[90%] mx-auto space-y-6 my-5">
              {examResult.savedAnswers.length > 0 ? (
                  ...
                  >
                    <CardContent className="p-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {t('examResults.questions.question')} {index + 1}: {answer.question_id.question_text}
                        </h3>
                        <div className="flex items-center space-x-2">
                          <span className="text-gray-600 text-lg">{t('examResults.questions.points')} : </span>
                            ...
                            >
                              <div className="flex items-center">
                                ...
                                {isSelected && !isCorrect && (
                                  <p className="text-red-600 ml-4">{t('examResults.questions.correctAnswer')}: {answer.question_id.correct_answer}</p>
                                )}
                              </div>
                            </label>
                          );
                ...
                <p className="text-gray-600 h-[50vh] text-center flex items-center justify-center"> {t('examResults.questions.noAnswers')}</p>
              )}
            </div>
          </tbody>
        </table>
