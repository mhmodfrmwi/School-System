...
const ExamCard = ({ exam, onView, onEdit, onDelete, onViewResults }) => {
  ...
  return (
    <div className={`p-6 bg-slate-50 rounded-xl shadow-md border border-[#117C90] hover:shadow-lg transition-shadow duration-300 ${textAlign}`} dir={direction}>
    ...
    <div className="p-6 space-y-6" dir={direction}>
      ...
      <div className="grid font-poppins gap-6 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1">
        {sortedExams.length > 0 ? (
          ...
          <div className={`col-span-full flex flex-col items-center justify-center bg-[#F9FAFB] py-16 rounded-lg shadow-lg ${textAlign}`} dir={direction}>
            <FontAwesomeIcon
              icon={faCalendar}
