      ...
      <div className="grid grid-cols-1 justify-items-center gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3">
        {classTeachers.map((classteacher, index) => (
          <div
            ...
            onClick={() =>
              navigate(
                `/teacher/exam-score/${classteacher?.classId?._id}/${classteacher.id}/students`,
              )
            }
            className="relative flex w-64 cursor-pointer flex-col items-center rounded-xl border border-gray-300 bg-slate-100 p-5 text-center shadow-lg transition-colors hover:bg-slate-200 dark:bg-DarkManager2 dark:hover:bg-[#2A2A2A]"
          >
...
export default CurrentCoursesForGrades;